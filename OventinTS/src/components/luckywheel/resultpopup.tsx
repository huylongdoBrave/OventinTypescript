...
const ResultPopup: React.FC<ResultPopupProps> = ({ isOpen, prize, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-[1001]
    transition-opacity duration-300 ease-in-out">
        ...
        >
            ...
            />
            <p className="text-lg md:text-[1.5rem] font-black m-0 min-h-[50px] text-[#233da3]">{prize?.name || 'Chúc bạn may mắn lần sau'}</p>
            <small className="text-lg md:text-[1.5rem] font-black m-0 min-h-[50px] text-[#233da3]">{prize ? `_ ID: ${prize.id}` : ''}</small>
            <button 
                ...
                onClick={onClose}>Tiếp tục quay</button>
        </div>
        <img 
            src="/static/boy.png" 
            alt="Boy" 
